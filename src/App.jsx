...
// Importamos los datos de las mesas y el componente de éxito
...
function MainLanding() {
  ...
  const cargarOcupados = async () => {
    try {
      ...
      if (data.suites && data.mesas) {
        setSuitesOcupadas(data.suites);
        setMesasOcupadas(data.mesas);
      } else {
  ...
  useEffect(() => {
    cargarOcupados(); 
    const intervalo = setInterval(cargarOcupados, 2000);
  ...
  const handleTablePayment = async () => {
    ...
    try {
      console.log(`🛒 Iniciando pago para Mesa #${selectedTable.id}`);
      const response = await fetch(`${API_URL}/create-checkout-session`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-[100] bg-black/98 backdrop-blur-2xl overflow-y-auto animate-fade-in p-4 md:p-10">
          <div className="max-w-7xl mx-auto relative bg-stone-900/50 rounded-[3rem] border border-white/10 p-6 md:p-12 shadow-2xl">
            <button onClick={() => {setSelectedEvent(null); setSelectedNumber(""); setSelectedTable(null); setSelectedTableCategory(null);}} className="absolute top-8 right-8 text-stone-500 hover:text-white transition-all hover:rotate-90"><X size={40} /></button>
            ...
            <div className="mt-20 pt-10 border-t border-white/10">
              <div className="flex items-center gap-3 text-amber-500 font-bold text-[10px] uppercase tracking-[0.4em] mb-2">
                <Gem size={16} /> 3. ¿Deseas agregar una Mesa VIP? (15 de Feb)
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-5 gap-12">
                ...
                <div className={`lg:col-span-3 bg-black/40 p-8 rounded-[3rem] border border-white/5 transition-all duration-700 ${!selectedTableCategory ? 'opacity-1 grayscale' : 'opacity-100'}`}>
